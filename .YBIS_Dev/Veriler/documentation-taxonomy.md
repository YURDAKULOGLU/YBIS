...
## 🔄 DUAL-WRITE RULE (CRITICAL INTEGRATION)

### Session Context ↔ Development Log Synchronization

**⚠️ ZORUNLU KURAL: Her session-context.md update'i Development Log entry gerektirir!**

```yaml
Rule Definition:
  trigger: "session-context.md file update"
  actions:
    - Update session-context.md (max 100 lines)
    - Simultaneously add entry to DEVELOPMENT_LOG.md
    - Ensure AD-XXX numbers match
    - Ensure dates match
    - No duplicate information

Purpose:
  session-context.md: "What's happening NOW?" (summary, quick load)
  DEVELOPMENT_LOG.md: "What happened?" (detailed history, permanent record)

Format:
  session-context:
    - Last 3 decisions (AD-XXX with date + one-line summary)
    - Next 3 steps (actionable tasks)
    - Current blockers (if any)
    - Total: ~100 lines

  DEVELOPMENT_LOG:
    - Day X entry with tasks completed
    - Architecture Decision full details (AD-XXX)
    - Context, rationale, impact, related decisions
    - Total: Detailed as needed

Enforcement:
  - AI agents MUST perform both updates
  - Checklist: [ ] session-context.md updated
  - Checklist: [ ] DEVELOPMENT_LOG.md entry added
  - Validation: AD-XXX numbers sequential and consistent
```
...
- **Decision:** Card-based design with priority/time/progress
...
```yaml
Without Dual-Write:
  - Agent reads DEVELOPMENT_LOG (2088 lines, ~10K token)
  - Just to understand current state

With Dual-Write:
  - Agent reads session-context.md (100 lines, ~500 token)
  - Gets current state instantly
  - Token savings: ~95%! 🎉
```
